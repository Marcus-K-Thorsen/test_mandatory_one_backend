...
class GeneratePhoneNumber:
    ...
    def generate(prefixes: list = VALID_PHONE_PREFIXES, randomDigitRange: tuple = (0, 9), max_length: int = 8):
        """Generate a random phone number."""
        phoneNumber = random.choice(prefixes)
        prefix_length = len(phoneNumber)
        for _ in range(max_length - prefix_length):
            phoneNumber += str(random.randint(randomDigitRange[0], randomDigitRange[1]))
        return phoneNumber
